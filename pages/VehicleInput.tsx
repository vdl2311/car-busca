...
const VehicleInput: React.FC = () => {
    ...
    if (view === 'dashboard') {
        return (
            <div className="flex flex-col min-h-full bg-background-dark p-6 md:p-12 max-w-7xl mx-auto page-transition safe-top space-y-12">
                <header>
                    <div className="flex items-center gap-3 mb-3">
                        <span className="material-symbols-outlined text-orange-500 font-bold">verified</span>
                        <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-500">AutoIntel Specialist v4.5</p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <button 
                        ...
                        className="group relative bg-surface-dark/40 border-2 border-white/5 p-10 md:p-16 rounded-[3.5rem] text-left hover:border-orange-500/50 hover:bg-orange-600/5 transition-all shadow-2xl"
                    >
                        ...
                        <h2 className="text-4xl font-black text-white uppercase italic tracking-tighter mb-4">Novo Check-in</h2>
                    ...
                    <button 
                        ...
                        className="group relative bg-surface-dark/40 border-2 border-white/5 p-10 md:p-16 rounded-[3.5rem] text-left hover:border-primary/50 hover:bg-primary/5 transition-all shadow-2xl"
                    >
                        ...
                        <h2 className="text-4xl font-black text-white uppercase italic tracking-tighter mb-4">Mecânico Virtual</h2>
                ...
                {/* HISTÓRICO RÁPIDO */}
                <section className="bg-white/[0.03] p-10 rounded-[3rem] border border-white/5 flex flex-col items-center justify-center py-20 text-center opacity-20">
                    <span className="material-symbols-outlined text-7xl mb-6">history</span>
                    <h3 className="text-xl font-black uppercase italic tracking-widest">Acesso Rápido</h3>
                    <p className="text-sm font-bold uppercase tracking-widest mt-2">Nenhum laudo recente para exibir</p>
                </section>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col min-h-full bg-background-dark p-6 md:p-12 max-w-7xl mx-auto page-transition safe-top">
            <header className="mb-12 flex items-center gap-8">
                <button onClick={() => setView('dashboard')} className="size-16 rounded-2xl bg-white/5 flex items-center justify-center text-slate-400 hover:text-white transition-colors">
            ...
            <div className="space-y-8">
                {/* IDENTIFICAÇÃO */}
                <section className="bg-surface-dark/40 border border-white/5 rounded-4xl p-8 md:p-10 space-y-8 relative z-30">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-3 relative" ref={brandRef}>
                            <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Marca</label>
                            <input 
                                type="text"
                                value={brandQuery}
                                ...
                                className="w-full h-20 bg-background-dark border-2 border-white/5 rounded-3xl px-6 text-xl font-black text-white focus:border-orange-500 transition-all placeholder:text-slate-800"
                            />
                            {showBrands && (
                                <div className="absolute top-full left-0 right-0 mt-3 max-h-72 overflow-y-auto bg-surface-dark border border-white/10 rounded-3xl z-[100] glass shadow-3xl">
                                    {filterItems(brands, brandQuery).map(b => (
                                        <div key={b.codigo} onClick={() => { setSelectedBrand(b); setBrandQuery(b.nome.toUpperCase()); setShowBrands(false); }} className="px-8 py-5 hover:bg-orange-600 hover:text-white cursor-pointer font-black text-sm uppercase tracking-wide border-b border-white/5 last:border-0">
                                            {b.nome}
                                        </div>
                                    ))}
                                </div>
                        ...
                        <div className="space-y-3 relative" ref={modelRef}>
                            ...
                            <input 
                                ...
                                className="w-full h-20 bg-background-dark border-2 border-white/5 rounded-3xl px-6 text-xl font-black text-white focus:border-orange-500 transition-all disabled:opacity-20"
                            />
                            {showModels && models.length > 0 && (
                                <div className="absolute top-full left-0 right-0 mt-3 max-h-72 overflow-y-auto bg-surface-dark border border-white/10 rounded-3xl z-[100] glass shadow-3xl">
                                    {filterItems(models, modelQuery).map(m => (
                                        <div key={m.codigo} onClick={() => { setSelectedModel(m); setModelQuery(m.nome.toUpperCase()); setShowModels(false); }} className="px-8 py-5 hover:bg-orange-600 hover:text-white cursor-pointer font-black text-sm uppercase tracking-wide border-b border-white/5 last:border-0">
                                            {m.nome}
                                        </div>
                                    ))}
                                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div className="space-y-3 relative" ref={yearRef}>
                            <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Ano Modelo</label>
                            <input 
                                type="text"
                                value={yearQuery}
                            ...
                            {showYears && years.length > 0 && (
                                <div className="absolute top-full left-0 right-0 mt-3 max-h-72 overflow-y-auto bg-surface-dark border border-white/10 rounded-3xl z-[100] glass shadow-3xl">
                                    {filterItems(years, yearQuery).map(y => (
                                        <div key={y.codigo} onClick={() => { setSelectedYear(y); setYearQuery(y.nome.toUpperCase()); setShowYears(false); }} className="px-8 py-5 hover:bg-orange-600 hover:text-white cursor-pointer font-black text-sm uppercase tracking-wide">
                                            {y.nome}
                                        </div>
                                    ))}
                                </div>
                        ...
                        <div className="space-y-3">
                            <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Motorização</label>
                            <input type="text" value={engine} onChange={(e) => setEngine(e.target.value)} placeholder="1.0 TSI / 2.0 8V" className="w-full h-20 bg-background-dark border-2 border-white/5 rounded-3xl px-6 text-xl font-black text-white" />
                        </div>
                        <div className="space-y-3">
                            <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">KM Atual</label>
                            <input type="number" value={mileage} onChange={(e) => setMileage(e.target.value)} placeholder="85000" className="w-full h-20 bg-background-dark border-2 border-white/5 rounded-3xl px-6 text-xl font-black text-white" />
                        </div>
                    </div>
                </section>

                <section className="bg-surface-dark/40 border border-white/5 rounded-4xl p-8 md:p-10 relative z-10">
                    <div className="space-y-3">
                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">O que o carro está apresentando? (Sintomas)</label>
                        <textarea 
                            value={symptoms}
                            onChange={(e) => setSymptoms(e.target.value)}
                            placeholder="Ex: Barulho na suspensão ao passar em irregularidades, falhando em baixa rotação, luz do óleo piscando..."
                            className="w-full h-32 bg-background-dark border-2 border-white/5 rounded-3xl px-6 py-5 text-lg font-bold text-white focus:border-orange-500 outline-none transition-all resize-none"
                        />
                    </div>
                </section>
